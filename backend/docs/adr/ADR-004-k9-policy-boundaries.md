...
| Price intelligence collection | Disabled by default; operator-only; requires robots allowed and terms accepted with evidence | `/operator/price-intelligence/*` | Source legal approval, collection activation |
| Price intelligence FX/customer presentation | No customer endpoint; no cross-currency savings without approved freshness policy | Customer competitor-price UI | FX freshness threshold and customer display policy |

...
def test_exact_grams_input_preserves_legacy_and_new_contracts() -> None:
    settings = Settings()
    legacy = _remaining_facts(OpenInventoryBody(remaining_grams=2100), settings)
    modern = _remaining_facts(
...
def test_level_input_validates_then_fails_closed_without_bounds_policy() -> None:
    body = OpenInventoryBody(
        remaining={"mode": "level", "level": "more_than_half"},
        daily_portion_grams=85,
    )
    with pytest.raises(HTTPException) as exc_info:
        _remaining_facts(body, Settings())
    assert exc_info.value.status_code == 409
    assert exc_info.value.detail == "semantic_level_policy_disabled"

...
@router.get("/breeds", response_model=dict[str, object])
...
async def search_breeds(
    ...
    return {
        "release": {"dataset_version": release.dataset_version},
        "items": [
            {
...
@router.get("/breeds/{breed_id:path}", response_model=dict[str, object])
...
def _public_source(source: KnowledgeSource) -> dict[str, object]:
    allowed = {
...
@router.get("/pets/{pet_id}", response_model=dict[str, object])
async def pet_knowledge(
    ...
    breed = detail["breed"]
    if not isinstance(breed, dict) or breed.get("species") != pet.species:

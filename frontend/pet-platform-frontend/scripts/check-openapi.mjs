...
for (const [name, value] of Object.entries(actual)) {
  if (contract[name] !== value)
    differences.push(`${name} expected ${contract[name]} actual ${value}`);
}
...
for (const [name, properties] of Object.entries(
  ...
  if (!schemas[name]) differences.push(`missing schema ${name}`);
    ...
    for (const property of properties)
      if (!schemas[name].properties?.[property])
        differences.push(`${name} missing ${property}`);
}
...
const variants = (value) =>
  [value, ...(value?.anyOf ?? []), ...(value?.oneOf ?? [])].filter(Boolean);
for (const [qualified, expected] of Object.entries(contract.required_enums)) {
  const values = variants(property(qualified)).flatMap(
    (entry) => entry.enum ?? [],
  );
  for (const value of expected)
    if (!values.includes(value))
      differences.push(`${qualified} enum missing ${value}`);
}
for (const [qualified, expected] of Object.entries(
  contract.required_patterns,
)) {
  if (property(qualified)?.pattern !== expected)
    differences.push(`${qualified} pattern mismatch`);
}
for (const [qualified, expected] of Object.entries(
  contract.required_constants,
)) {
  if (!variants(property(qualified)).some((entry) => entry.const === expected))
    differences.push(`${qualified} constant mismatch`);
}
...
try {
  ...
  if (current !== fresh)
    throw new Error("generated OpenAPI types drifted; run pnpm generate:api");
} finally {
  rmSync(tempDir, { recursive: true, force: true });
}
console.log(
  JSON.stringify(
    { status: "compatible", alembic_heads: manifest.alembic.heads, ...actual },
    null,
    2,
  ),
);

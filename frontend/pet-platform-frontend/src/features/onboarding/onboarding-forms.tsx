...
export function HouseholdOnboardingForm() {
  ...
  return (
    <OnboardingScreen eyebrow="فعال‌سازی Pet Life" title="نام خانوار">
      <Card>
        <form className="stack" onSubmit={form.handleSubmit(onSubmit)}>
          <Input
            id="household-name"
...
export function PetOnboardingForm() {
  ...
  async function onSubmit(values: z.infer<typeof petSchema>) {
    const householdId = getOnboardingProgress().householdId;
    if (!householdId) {
      setSubmitError("شناسه خانوار پیدا نشد. از مسیر فعال‌سازی ادامه دهید.");
      return;
    }
    setSubmitError(null);
    try {
...
export function PetBirthDateForm() {
  ...
  async function submitBirthDate(values: z.infer<typeof birthDateSchema>) {
    const petId = getOnboardingProgress().petId;
    if (!petId) {
      setSubmitError("شناسه پت پیدا نشد. از مسیر فعال‌سازی ادامه دهید.");
      return;
    }
    setSubmitError(null);
    try {
